  ...
  if (any(notNA_counts >= 5)) {
    ...
    for (var in vars) {
      ...
      if (notNA_count >= 5) {
        ...
        lm_model <- lm(lm_formula, data = temp_data) %>% summary()
...
``` {r final model, include = F, echo = F, message = F, warning = F}
...
for (boot in 1:1000) {
  ...
  model_data <- boot_data %>%
    rowwise() %>%
    mutate(PID7_slope = PID7_slope + rnorm(1, mean = PID7_slope, sd = PID7_slopeError),
           ACA_2008 = ACA_2008 + rnorm(1, mean = ACA_2008, sd = ACA_2008Error),
           ACA_slope = ACA_slope + rnorm(1, mean = ACA_slope, sd = ACA_slopeError),
           IDEO7_2008 = IDEO7_2008 + rnorm(1, mean = IDEO7_2008, sd = IDEO7_2008Error),
           IDEO7_slope = IDEO7_slope + rnorm(1, mean = IDEO7_slope, sd = IDEO7_slopeError),
           PID7_2008 = PID7_2008 + rnorm(1, mean = PID7_2008, sd = PID7_2008Error),
           SYSLEG_slope = SYSLEG_slope + rnorm(1, mean = SYSLEG_slope, sd = SYSLEG_slopeError)) %>%
    select(-matches('Error'))
  
  ### Regress
  lm_model <- lm(PID7_slope ~ ACA_2008 + ACA_slope + IDEO7_2008 + IDEO7_slope + 
                   PID7_2008 + SYSLEG_slope, data = model_data) %>% summary()
  coefs <- lm_model$coefficients %>% as.data.frame()
...
Using this approach, we find that none of the bootstrapped 95% confidence intervals for the variables in our final model overlap with zero. This confirms the results of our original linear model: our coefficients are significantly different from zero.
